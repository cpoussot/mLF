function [c_new,curr_new] = bary_data2cN(c,curr)
...
for jj = 1:length(curr)
    eval(['c_new.s' num2str(n) ' = [c_new.s' num2str(n) ' curr{jj}.c' num2str(n) '];']);
    if isfield(curr{jj},'i2')
        [c1,c2]     = mlf.bary_data2c12(curr{jj},c1,c2);
        c_new.s1    = c1;
        c_new.s2    = c2;
    else
        eval(['curr_new = curr{' num2str(jj) '}.i' num2str(n-1) ';']);
        [c_new,curr_new] = mlf.bary_data2cN(c_new,curr_new);
    end
end